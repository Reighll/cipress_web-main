...
<head>
    ...
    <title>Owner Registration - N.J Cipress</title>
...
<div class="container mx-auto p-4">
    <div class="flex flex-col items-center justify-center">
        ...
        <div class="w-full max-w-lg">
            <h2 class="text-4xl font-bold text-gray-800 text-center mb-8">CREATE ACCOUNT</h2>
            <div class="bg-gray-800 rounded-2xl shadow-lg p-8 md:p-12 w-full">
                ...
                <form id="registration-form" method="post">
                    <div class="space-y-4">
                        ...
                        <div id="system-key-container">
                            <input type="text" name="system_key" placeholder="Enter System Key" class="form-input">
                            <div id="error-system_key" class="error-message"></div>
                        </div>
                    </div>
                    <div class="flex justify-between items-center mt-8">
                        <a href="/register" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-8 rounded-lg transition duration-300 inline-block">
                            Back
                        </a>
...
<script>
    ...
    form.addEventListener('submit', async function(event) {
        ...
        try {
            const response = await fetch('/register/owner', {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest' // Important for CI4 to recognize it as AJAX
                },
                body: formData
            });
            ...
            if (!response.ok) {
                ...
                if (result.messages) {
                    ...
                    notificationArea.innerHTML = `<div class="p-4 bg-red-500 text-white rounded-lg">An unknown error occurred. Please check server logs.</div>`;
                }
            } else {
                // Handle success
    ...
    // --- System Key Field Visibility ---
    const ownerCount = <?= $ownerCount ?? 0 ?>;
    const systemKeyContainer = document.getElementById('system-key-container');

    // Show the system key field only if there are existing owners.
    if (ownerCount > 0) {
        systemKeyContainer.style.display = 'block';
    }

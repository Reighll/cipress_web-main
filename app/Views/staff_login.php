...
<div class="container mx-auto p-4">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        ...
        <div class="bg-gray-800 rounded-2xl shadow-lg p-8 md:p-12 max-w-md mx-auto w-full">
            <h2 class="text-3xl font-bold text-white text-center mb-8">STAFF LOGIN</h2>
            ...
            <form id="login-form" method="post">
                <div class="space-y-6">
                    ...
                    <div class="flex items-center"><input type="checkbox" id="show_password" class="mr-2"><label for="show_password" class="text-gray-400">Show Password</label></div>
                </div>
                <div class="flex justify-between items-center mt-8">
                    <a href="/" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-8 rounded-lg">Back</a>
                    <button type="submit" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-8 rounded-lg">Log in</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    document.getElementById('show_password').addEventListener('change', function() { document.getElementById('password').type = this.checked ? 'text' : 'password'; });
    document.getElementById('login-form').addEventListener('submit', async function(e) {
        e.preventDefault();
        document.getElementById('notification').innerHTML = '';
        const formData = new FormData(this);
        const res = await fetch('/login/staff', { method: 'POST', headers: {'X-Requested-With': 'XMLHttpRequest'}, body: formData });
        const result = await res.json();
        if (!res.ok) {
            document.getElementById('notification').innerHTML = `<div class="p-4 bg-red-500 text-white rounded-lg">${result.messages.error}</div>`;
        } else {
